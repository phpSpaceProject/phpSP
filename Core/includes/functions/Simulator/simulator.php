...
function simulator($CurrentSet, $TargetSet, $CurrentTechno, $TargetTechno) {
	...
	for ($i = 0; $i < 6; $i++) {
		...
		if (!is_null($TargetSet)) {
			...
			$defenseur_nombre = 0;
			$runda[$i]["attaquant"] = $CurrentSet;
			$runda[$i]["defenseur"] = $TargetSet;
			$runda[$i]["attaquant"]["atak"] = $attaquant_dommage;
			$runda[$i]["defenseur"]["atak"] = $defenseur_dommage;
			$runda[$i]["attaquant"]['count'] = $attaquant_nombre;
			$runda[$i]["defenseur"]['count'] = $defenseur_nombre;
			break;
		}
	...
	for($a = 200; $a < 500; $a++) {
		...
		$runda[$i]["attaquant"]["tarcza"] = $attaquant_bouclier;
		$runda[$i]["defenseur"]["tarcza"] = $defenseur_bouclier;
	...
	$zlom["attaquant"] = (($attaquant_structure['metal'] - $attaquant_perte['metal']) + ($attaquant_structure['crystal'] - $attaquant_perte['crystal']));
	$zlom["defenseur"]      = (($defenseur_structure['metal']      - $defenseur_perte['metal'])      + ($defenseur_structure['crystal']      - $defenseur_perte['crystal']) + $straty_coque_defenseur);
	return array("attaquant" => $CurrentSet, "defenseur" => $TargetSet, "wygrana" => $wygrana, "dane_do_rw" => $runda, "zlom" => $zlom);
}
